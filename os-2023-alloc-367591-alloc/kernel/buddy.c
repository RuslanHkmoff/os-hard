...
void bit_flip(char *array, int index) {
  if (bit_isset(array, index / 2)) {
    bit_clear(array, index / 2);
    return;
  }
  bit_set(array, index / 2);
}

int bit_isset_shift(char *array, int index) {
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].xor, NBLK(k));
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_flip(bd_sizes[k].xor, blk_index(k, p));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_flip(bd_sizes[k - 1].xor, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_flip(bd_sizes[k].xor, bi);                  // free p at size k
    if (bit_isset_shift(bd_sizes[k].xor, buddy)) {  // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, int flag) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset_shift(bd_sizes[k].xor, bi)) {
