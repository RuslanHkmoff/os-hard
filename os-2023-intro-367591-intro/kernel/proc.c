...
int dump2(int pid, int register_num, uint64 *return_value) {
  ...
  for (; p < &proc[NPROC]; p++) {
    if (p->pid == pid) {
      if (p == curr || p->parent == curr) {
        char *reg = (char *)(&(p->trapframe->s2) + shift);
        int flag = copyout(curr->pagetable, *return_value, reg, strlen(reg));
        if (flag == 0) {
          return 0;
        }
        return -4;
